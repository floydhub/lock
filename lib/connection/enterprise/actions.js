...
var throwIfOidcIsEnabledAndConnectionIsADAndHasUsername = function throwIfOidcIsEnabledAndConnectionIsADAndHasUsername(
  m,
  connection,
  hasUsername
) {
  var isOIDC = l.oidcConformant(m);
  var isAD = ['ad', 'adfs', 'office365', 'waad'].indexOf(connection.get('strategy')) >= 0;
...
function logIn(id) {
  ...
  var email = (0, _index2.getFieldValue)(
    m,
    (0, _index4.databaseLogInWithEmail)(m) ? 'email' : 'username'
  );
...
function logInActiveFlow(id) {
  var m = (0, _index.read)(_index.getEntity, 'lock', id);
  var usernameField =
    (0, _enterprise.isHRDActive)(m) || !(0, _index4.databaseLogInWithEmail)(m)
      ? 'username'
      : 'email';
  ...
  var username = l.defaultADUsernameFromEmailPrefix(m)
    ? (0, _email.emailLocalPart)(originalUsername)
    : originalUsername;
